    ...
    return menuTreeN.map(item => {
      ...
      return (
        <Menu.Item key={item.id} className={styles.item} >
          <Link to={item.router} >
            {item.icon && <Icon type={item.icon} style={{ width: '39.4px', height: '48.9px', fontSize: '50px', position: 'absolute', top: '37px', left: '45.5px', color: darkTheme ? '#ffffff' : '#000000' }} />}<br />
          </Link>

          <span>
            <Link to={item.router} style={{ paddingLeft: '14px', color: darkTheme ? '#ffffff' : '#000000' }}>
              {(!siderFoldN || menuTree.indexOf(item) < 0) && item.name}
            </Link>
          </span>
        </Menu.Item>
