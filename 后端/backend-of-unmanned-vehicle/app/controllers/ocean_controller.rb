...
class OceanController < ApplicationController
  ...
  def connet_mongdb1 #连接mongodb，只连接一个
  ...
  # 同步文件方法，保持 equipment_csv 和 bd09_csv 文件夹下文件一致
  def w_bd09
    # 保障 bd09 文件夹不会有多余的内容
  ...
  def update_latest
    ...
    Dir.foreach("#{Rails.root}/equipment_csv") do |filename|
      # 去除非时间命名格式文件
      if filename != '.' && filename != '..' && filename != 'test1015-2.csv' && filename != 'planned_path.csv'
...
=end
  ...
  class << self
    ...
    def connet_mongdb #连接mongodb
      client = Mongo::Client.new('mongodb://192.168.21.102:27017/ugv')
